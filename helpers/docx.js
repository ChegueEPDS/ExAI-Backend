...
/**
 * 📦 DOCX buffer generálása (fájlírás nélkül)
 * Megőrzi a generateDocxFile formázását, de nem ír lemezre – Buffer-t ad vissza.
 * @param {string} ocrText - Az OCR által felismert nyers szöveg
 * @param {string} certNo - A tanúsítvány száma (címhez)
 * @param {string} certType - A tanúsítvány típusa ("ATEX" vagy "IECEx")
 * @returns {Promise<Buffer>} - A DOCX tartalma Bufferben
 */
async function generateDocxBuffer(ocrText, certNo, certType) {
    try {
        console.log(`📄 DOCX buffer generálása... Típus: ${certType}`);

        // 🚀 1. Szöveg előkészítése és tisztítása (azonos a fájlos verzióval)
        let cleanedText = ocrText;
        ...
        // 🚀 3. Szöveg feldolgozása (azonos logika)
        lines.forEach((line) => {
            if (certType === "ATEX") {
                const atexMatch = line.match(atexHeadingRegex);
                if (atexMatch) {
                    formattedParagraphs.push(new Paragraph({
                        children: [
                            new TextRun({ text: `📌 ${atexMatch[3]}: `, bold: true, color: "0000FF" }),
                            new TextRun({ text: atexMatch[4] || "" })
                        ],
                        spacing: { after: 200 }
                    }));
                    return;
        ...
        // 📦 Bufferben adjuk vissza
        const buffer = await Packer.toBuffer(doc);
        return buffer;
    } catch (error) {
        console.error("❌ Hiba DOCX buffer generálás közben:", error);
        throw error;
    }
}

module.exports = { generateDocxFile, generateDocxBuffer };