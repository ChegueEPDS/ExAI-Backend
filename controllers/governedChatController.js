  ...
  try {
    ...
    try {
      ...
      if (measEval.enabled() && tool === 'analyze_measurement_tables') {
        ...
        if (ev?.ok && ev?.result) {
          try { measEval.enrichMeasurementEvalResultForMarking(ev.result); } catch { }
          ...
          contextParts.push('NOTE: If result contains ts_by_point / tmax_by_point, these are COMPUTED values: Ta,max (ambient_field, e.g. "Max. allowed ambient temperature") + max(Ti−T12). Here T12 is the test ambient during measurement (per table time column). Use ts_by_point for Ts (Tables 1–4) and tmax_by_point for Tmax (Tables 5–8). Do not describe these as directly measured temperatures at Ta,max; they are extrapolated/derived.');
    ...
    let systemPartsHu = [
      ...
      'Ha a MEASUREMENT_EVAL_CONTEXT tartalmaz ts_by_point / tmax_by_point mezőket, ezeket COMPUTED értékként kezeld: Ta,max (ambient_field, pl. "Max. allowed ambient temperature") + max(Ti−T12). Itt a T12 a mérés alatti teszt-ambient (azonos időoszlopban). Ts-hez ts_by_point, Tmax-hoz tmax_by_point használható. Ne nevezd ezeket közvetlenül mért értéknek Ta,max-on; ezek extrapolált/derivált értékek.',
      'Ha a user Ex jelölést / hőmérséklet osztályt kér (gáz + por): (1) állapítsd meg a kontrolling hőmérsékletet: HA a MEASUREMENT_EVAL_CONTEXT.marking_inputs tartalmaz controlling_gas_accessible_tmax / controlling_dust_surface_tmax mezőket, akkor (gázhoz) EZT használd kontrolling Tmax-ként (csak gáz-atmoszférának hozzáférhető felületek), és (porhoz) a controlling_dust_surface_tmax-et (csak külső, porlerakódásnak kitett felületek). Fontos: ez a hozzáférhetőség egy HEURISZTIKA a pont-label alapján; ha nincs DOCUMENT_CONTEXT bizonyíték, akkor jelöld "ASSUMPTION" / "DRAFT" alapon. Ha a mezők hiányoznak, fallback: a pontok közül a legnagyobb Ts/Tmax. (2) a gáz T-osztályt a STANDARD_CONTEXT-ben lévő releváns táblázat/határérték alapján válaszd ki; (3) porra a kontrolling hőmérsékletből adj meg "T{X}°C" jelölést (max. felületi hőmérséklet), és KEREKÍTS FEL egész °C-ra; FONTOS: a "tb" nem hőmérséklet, hanem Ex t védelmi mód (IEC 60079-31), ezért ne írj olyat, hogy "tb90°C" — helyette (ha ismert a DOCUMENT_CONTEXT alapján) formázd így: "Ex tb … T{X}°C Db"; (4) a jelölés többi részét (csoport/kategória/védelmi mód/EPL) a DOCUMENT_CONTEXT alapján írd ki. Ha ezek HIÁNYOZNAK a DOCUMENT_CONTEXT-ből, akkor adhatsz egy külön "DRAFT / feltételezett" jelölést is, EXPLICIT FELTÉTELEZÉSEKKEL: vélelmezd, hogy Equipment Group = II és kategória = 2 (II 2G vagy II 2D); ennek megfelelő EPL: 2G → Gb, 2D → Db; védelmi módnak javasolj gázra "Ex eb", porra "Ex tb". A csoport (IIA/IIB/IIC, IIIA/IIIB/IIIC), IP, Ta, és a tanúsítvány száma maradjon UNKNOWN/placeholder (pl. "IIC? / IIIC? / IP?? / Ta?? / Cert??") hacsak nincs rá bizonyíték. A DRAFT jelölést mindig jelöld meg úgy, hogy "csak javaslat / ellenőrizendő névtábla alapján".',
    ...
    let systemPartsEn = [
      ...
      'If MEASUREMENT_EVAL_CONTEXT includes ts_by_point / tmax_by_point, treat them as COMPUTED values: Ta,max (ambient_field, e.g. "Max. allowed ambient temperature") + max(Ti−T12). Here T12 is the test ambient during measurement (same time column). Use ts_by_point for Ts and tmax_by_point for Tmax. Do not call these directly measured temperatures at Ta,max; they are extrapolated/derived.',
      'If the user asks to determine Ex marking / temperature class (gases + dusts): (1) identify the controlling temperature: if MEASUREMENT_EVAL_CONTEXT.marking_inputs contains controlling_gas_accessible_tmax / controlling_dust_surface_tmax, use those as the controlling Tmax for (gas) and (dust) respectively (gas-accessible vs dust-deposit external surfaces). IMPORTANT: this accessibility is a heuristic based on point labels; if DOCUMENT_CONTEXT proof is missing, label it clearly as ASSUMPTION / DRAFT. If missing, fallback to the maximum across all points and across Ts/Tmax; (2) determine the gas temperature class using the relevant standards table/limits from STANDARD_CONTEXT; (3) for dust, output a dust surface temperature marking as "T{X}°C" derived from the controlling temperature and ROUND UP to a whole °C. IMPORTANT: "tb" is not a temperature; it is the Ex t protection level (IEC 60079-31). Do NOT write "tb90°C". If the protection mode / EPL is known from DOCUMENT_CONTEXT, format like "Ex tb … T{X}°C Db"; (4) include the rest of the marking (group/category/protection/EPL) using DOCUMENT_CONTEXT when available. If those fields are MISSING from DOCUMENT_CONTEXT, you may additionally provide a separate "DRAFT / assumed" marking with EXPLICIT assumptions: assume Equipment Group = II and category = 2 (II 2G or II 2D); corresponding EPL: 2G → Gb, 2D → Db; suggest protection type "Ex eb" for gas and "Ex tb" for dust. Keep gas/dust group (IIA/IIB/IIC, IIIA/IIIB/IIIC), IP, Ta, certificate number as explicit UNKNOWN placeholders (e.g., "IIC? / IIIC? / IP?? / Ta?? / Cert??") unless evidenced. Always label the DRAFT marking as "needs verification against nameplate/certificate".',
